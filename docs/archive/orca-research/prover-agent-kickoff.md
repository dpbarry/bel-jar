# Prover agent kickoff — archived

> **Archived** (pre-dates the 2026-08-29 shelf). Older onboarding that still says
> "Harpoon autosolve". Start at [`README.md`](README.md). Shipped engine: [`../../ORCA.md`](../../ORCA.md).
...
READ (in order): prover-master-plan.md (whole), this file, then
...
## 0.4 ⭐⭐ CURRENT STATE — READ THIS BEFORE §0.5/§0.6 (2026-08-22, master plan entry 69)

**The live arc is HOLE-DIRECTED REFINEMENT** (entries 60 → 69), not any of the term-generation
mechanisms below. `%:checkinhole` lets Beluga type an INCOMPLETE term in the hole's own
context and hand back the argument goals; the search proposes `c ? … ?` and recurses.

- ⭐⭐⭐ **The checker enumerates the inhabitants.** Every subgoal `checkinhole` registers is a
  real hole, so `%:printhole` on it prints **`Variables of this type: MS1'`** — the
  type-correct in-scope names, already spelled for that level (a Γ name bare at computation
  level, a Δ name bare inside a box). Reading that line is the single change that took the
  solved-leaf control from 2/5 to 67/75. **Do not hand-build a candidate pool for R7.**
- ⛔⛔ **NO NUMBER FROM THIS ARC COUNTS UNTIL `scratch/probes/ls3-verify.mjs` HAS RUN.**
  `checkinhole` elaborates with no enclosing theorem (`mfs = []`) and the harness strips
  `/ total /`, so `OK 0` is WELL-TYPED, never TERMINATING. Unguarded, the search closes
  leaves with `complete ms` / `nbe t` — the theorem applied to its own untouched inputs.
  The verifier splices the term back, RESTORES `/ total /`, and reloads the declaration.
  It moved the control 89.3% → **74.7%** and the residue 47.2% → **36.1%**.
- ⛔⛔ **Two harness traps in the primitive itself, each of which faked a verdict:** the hole
  store ACCUMULATES (subgoals are never released), and hole IDS ARE SPARSE (a failed
  `checkinhole` burns ids, so `countholes - n` addresses holes that never existed and
  `printhole` answers *"No such hole by id"*). Address the newest holes by scanning upward
  from the last live id.
- **Where it stands, honestly.** Below entry 61b's ~10-token cliff, leaf synthesis is solved:
  **60% of small residue leaves verify**, median 4–8 checker calls. Above it, **1/16** — the
  cliff is intact and remains the wall. **The ledger is unchanged at 273/850**: every run
  splices the author's own prefix(maxDepth-1), so the structure is handed over and nothing
  here is a corpus completion yet.
- **Next, in order:** (1) the unwritable context variable — `%:printhole` says `y`, the
  source says `g`, and the declaration rejects `[y |- …]`; (2) compose oracle-STRUCTURE
  (entry 59) with oracle-LEAVES — the one untested cell of entry 60.4's table; (3) split the
  15 unverified large leaves into TERMINATION / WRITABILITY / genuinely-unfound.
- **Instruments:** `ci-ask.mjs` (ask the primitive any expression at a leaf — start here,
  it tests a spelling hypothesis in one command) · `ls2-diag.mjs` (which candidate is
  missing, and why) · `leaf-synth3.mjs` · `ls3-score.mjs` · `ls3-verify.mjs`.
- ⛔ **Do not re-try iterative deepening.** Measured negative twice (entries 68, 69): 0 gains,
  3–19× the calls.

---


### ⭐⭐⭐ THE CEILING NUMBER (entry 70.4) — read this before proposing any refinement work

The composition test is done. Handing over the author's own induction, case tree, contexts
and bound hypotheses **for free**, and synthesising every leaf with the oracle-directed
search, then checking the whole declaration with `/ total /` restored:

| whole-target composition | solved control | **residue** |
|---|---|---|
| declaration verifies | 55 / 73 (75.3%) | **3 / 23 (13.0%)** |

⛔ **Read it as a CEILING.** 13% of the hardest residue class, with a PERFECT structure
oracle it does not have (entry 59 measured structure synthesis at 0/45). Against the
494-target residue that is ≈64 targets ≈ **+7.5 points of corpus**, gated on a second
unsolved problem. **The composed refinement design does not close the mandate's arithmetic.**

⭐ **Where it dies is now known: 13 of 14 targets that closed EVERY leaf and still failed the
declaration failed for TERMINATION** ("Recursive call") — the search makes recursion it is
not entitled to make. Mandate §7: termination is OUR invariant. The engine's own
`decreasingArgIndex` model exists and the refinement search does not consult it. Highest
reach in the arc (14 targets), but stake it on CONVERSION, not on the 14.
...
**Instruments** live in `scratch/probes/` (rebuild if cleared, ~40 lines each):
...
- ⛔ **SIZE BY THE MECHANISM'S OWN PREDICATE, not by what proofs contain.** The reference-text
  ...
  **4/11**. `scratch/probes/mixedslot-reach.mjs` computes the structural class offline in
...
- ⭐ **THE BEST REMAINING SLICE, SIZED AND UNBUILT: context-structural induction, 16 targets,
  EXACT type-level predicate.** `scratch/probes/ctxind-census.mjs` → (A) explicit `{g:<schema>}`
  binder, (B) measure NAMES it, (C) reference splits `case [g] of`; **A+B = A+C = A+B+C = 16**,
  so the class is identifiable from the TYPE ALONE (ids in `scratch/probes/ctxind-ids.txt`).
...
- ⭐ **NEW INSTRUMENT, KEEP IT: `scratch/probes/error-census.mjs`** — runs a stride sample and
  ...
  Companion: `scratch/probes/ab-toggle.mjs --env <VAR>` A/Bs any engine toggle reporting BOTH
...
- **Library 269 COMPLETE measured.** Suite 212/212; differential 199/199.
...
| Path | Role |
|------|------|
| `prover-master-plan.md` | Single source of truth: direction, laws, ledger, every slice |
