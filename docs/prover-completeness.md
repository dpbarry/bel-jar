...
> **Not the direction SoT.** Absorbed into [`prover-master-plan.md`](prover-master-plan.md).
> Read the master plan for engine direction; keep this file only if you need the
> historical completeness-audit prose (D-divergences, enumerators obligation).
> If anything here conflicts with the master plan, the master plan wins.
