...
export function bindStackedDiagnosticTip(el, diags, { placement = 'right', T = null } = {}) {
  if (!el || !diags?.length) return;
  const tooltips = T ?? (typeof window !== 'undefined' && window.Tooltips);
...
function applyGutterTips(view, getBelugaDiags = null) {
  ...
  cells.forEach((cell) => {
    ...
    bindStackedDiagnosticTip(cell, diags, { placement: 'right', T });
  });
}
