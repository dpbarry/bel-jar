...
export function createEditHistory(adapter) {
  ...
  function beginEntry(kind, label) {
    flushTypingGroup();
    openEntry = {
      kind: kind || 'edit',
      label,
  ...
  function applyWorkspacePatch(entry, direction, rollback) {
    ...
    if (ed && activeId && touchedActive.has(activeId)) {
      ...
      const local = entry.editorLocal?.[activeId];
      let sel = selectionForFile(rec, direction, len);
      if (local?.selection && entry.kind === 'format') {
        sel = clampSelection(local.selection, len);
      }
      const ue = direction === 'undo' ? 'undo' : 'redo';
      if (typeof ed.replaceDocumentNonUndoable === 'function') {
        ed.replaceDocumentNonUndoable(target, {
          userEvent: ue,
          selection: sel,
        });
      } else {
        markNonUndoable();
        ed.setValueNonUndoable?.(target, { userEvent: ue, selection: sel })
          ?? ed.setValue?.(target);
      }
      adapter.syncActiveEditorCheckpoint?.(ed.getValue?.() ?? target);
      if (local && entry.kind === 'format' && ed.applyViewport) {
        queueMicrotask(() => {
          if (typeof ed.getValue === 'function' && ed.getValue() === target) {
            ed.applyViewport(local);
          }
        });
      }
    }
  ...
  function reconcileActiveFile(fileId, text) {
    ...
    entry.files[fileId] = { before: rec.before, after: text };
    persistStack();
  }
...
export function belEditHistoryKeymap() {
  return [
    {
      key: 'Mod-z',
      run: (view) => {
...
export function dispatchEdit(view, spec, meta = {}) {
  ...
  if (H && fileId && meta.kind) {
    H.flushTypingGroup();
    H.beginEntry(meta.kind, meta.label);
    H.touchFile(fileId);
...
export function belEditHistoryListener(fileId) {
