(() => {
  ...
  function applyValue(persist2, spec, requested) {
    if (!persist2 || !spec) return { ok: false, message: "Settings are not ready yet." };
    if (typeof persist2[spec.read] !== "function" || typeof persist2[spec.write] !== "function") {
      return { ok: false, message: `${spec.title} cannot be changed here.` };
    }
    const value = nextValue(spec, persist2[spec.read](), requested);
    if (value === null) return { ok: false, message: `${spec.title}: no such value.` };
    persist2[spec.write](value);
    return { ok: true, applied: true, spec, value, message: describeChange(spec, value) };
  }
  function runSetOn(persist2, raw) {
    ...
    return applyValue(persist2, res.spec, res.requested);
  }
