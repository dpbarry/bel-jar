...
var belugaBusy = false;
  ...
  async function runToHere(targetId) {
    ...
    // First suite member has no predecessors — same as a lone file; `&` only
    // when there is an actual amalgamation (prelude).
  ...
  function init() {
    if (typeof BelugaClient === 'undefined') {
      Toasts.error('Beluga client failed to load.', {
        ...
        closable: true,
        durable: true,
        body: 'The checker script never arrived, so nothing can be run or'
          + ' type-checked. Reload the page to retry.',
