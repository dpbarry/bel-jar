...
const WRAPPERS = {
  ...
  // Binder position: `(g:ctx)` alone is only grammatical when a type follows,
  // so append one — otherwise it re-parses as a parenthesized type and the
  // binder names lose their local-definition colour.
  binder: { pre: 'rec _belj : ', post: ' type = ?;' },
};
...
export function normalizeType(typeStr) {
  ...
  return String(typeStr)
    ...
    .replace(/=>/g, '⇒')
    .replace(/->/g, '→')
    // Canonical bracket hug — [g ⊢ o], [⊢ tp], (⊢ tp) — so the SAME type spelled
    // by the user ("[ ⊢ x ]"), by Beluga's printer ("[ ⊢ x]"), or by us always
    // renders identically and never jiggles when reconstruction supplants the
