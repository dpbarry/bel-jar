...
import { editorFileId } from '../ide/refs-panel.mjs';
import { dependencyGraph } from '../semantic/workspace-index.mjs';
import { createFollowWindowAction, registerEditorFollow } from '../ide/follow-sync.mjs';
import { computeParseCoverage, formatGlobalGraphStaleBanner } from '../ide/ide-status.mjs';
import { createForceSim } from './force-sim.mjs';
import { createGraph3D } from './webgl-graph.mjs';
import { createFlatGraph, renderFlatMini } from './flat-graph.mjs';
import { loadGraphPrefs, saveGraphPrefs, onGraphPrefsChange } from './graph-prefs.mjs';
import {
...
// Empty suites still return a source (empty nodes) — opening from a .cfg with
// nothing listed must open an empty graph, like an empty .bel file.
function makeGroupSource(view, engine) {
  ...
  const nodeMap = graph.nodes || new Map();
  ...
  return {
    ...
    graphFor: () => ({ nodes: [...nodeMap.values()], edges }),
...
// Beluga tree. Suite graphs (__group) are built from the workspace index over
// member texts — the active view's CM tree (especially a .cfg with no language)
// must not drive the stale banner.
function graphParseCoverage(view, engine = null) {
  if (engine?.__group || isCfgEditorView(view)) {
    return { parsed: 0, total: 0, percent: 100, complete: true };
  }
  if (!view) return null;
  return computeParseCoverage(view.state);
}

// Prefer the suite graph when available (including from a .cfg with no semantic
// engine). Fall back to the per-file engine for standalone / no-project cases.
function resolveGraphEngine(view) {
  const realEngine = getEngine(view);
  return makeGroupSource(view, realEngine) || realEngine || null;
}
...
function jumpToNode(view, engine, node) {
  ...
  const g = typeof window !== 'undefined' ? window : self;
  const activeId = g.Persist?.getActiveFileId?.();
...
function graphModelFingerprint(model) {
  ...
  const e = model.edges?.length ?? 0;
  // Sample ids so two different graphs with the same counts don't collide
  // (plain n:e was enough to skip needed remounts on tab switch).
  let sample = '';
  if (n) {
    sample = String(nodes[0]?.id ?? '');
    if (n > 1) sample += `>${nodes[n - 1]?.id ?? ''}`;
    if (n > 2) sample += `>${nodes[n >> 1]?.id ?? ''}`;
  }
  return `${n}:${e}:${model.root ?? ''}:${sample}`;
}
...
function bundleView(win) {
  return win?._graphBundle?.view ?? null;
}

function bundleEngine(win) {
  const bundle = win?._graphBundle;
  if (!bundle) return null;
  return bundle.engine || resolveGraphEngine(bundle.view);
}

/** Keep title + parse banner (+ idle prop parse %) in sync with the active file. */
function refreshGraphLoadingChrome(view, win, ctx, engine) {
  if (!view || !win || !ctx) return;
  const cur = navCurrent(win._graphNav);
  if (!cur) return;
  const parseCoverage = graphParseCoverage(view, engine);
  ...
  if (cur.mode === 'global') {
    ...
    if (ctx.viewMeta?.mode === 'global') {
      if (!parseCoverage.complete) ctx.viewMeta.parsePercent = parseCoverage.percent;
      else delete ctx.viewMeta.parsePercent;
      if (typeof meta.symbolCount === 'number') ctx.viewMeta.symbolCount = meta.symbolCount;
      if (!ctx.selectedNodeId) ctx.propBar?.showContext?.(ctx.viewMeta);
    }
  } else {
    // Local view: no parse banner, but drop any leftover global chrome from a prior file.
    updateStaleBanner(win.body, '');
    win.setTitle?.(graphWindowTitle());
  }
  return parseCoverage;
}

const LIVE_REMOUNT_MS = 400;

function clearLiveRemountTimer(ctx) {
  if (ctx?._liveRemountTimer) {
    clearTimeout(ctx._liveRemountTimer);
    ctx._liveRemountTimer = null;
  }
}

function performLiveRemount(view, win, ctx, engine, fp, parseDone) {
  const cur = navCurrent(win._graphNav);
  if (!cur) return;
  ctx._liveFp = fp;
  ...
  refreshGraphLoadingChrome(view, win, ctx, engine);
}

function scheduleLiveRemount(view, win, ctx, engine, fp, parseDone, { immediate = false } = {}) {
  ctx._pendingRemount = { view, engine, fp, parseDone };
  const run = () => {
    ctx._liveRemountTimer = null;
    const pending = ctx._pendingRemount;
    ctx._pendingRemount = null;
    if (!pending) return;
    const v = bundleView(win) || pending.view;
    const eng = bundleEngine(win) || pending.engine;
    if (!v || !eng) return;
    // Re-check against the latest model — a newer sync may have made this a no-op.
    const cur = navCurrent(win._graphNav);
    if (!cur) return;
    const prefs = loadGraphPrefs();
    const model = cur.mode === 'global'
      ? buildGlobalModel(eng)
      : buildNeighborhood(eng, cur.rootId, { depth: prefs.depth });
    if (!model) return;
    const latestFp = graphModelFingerprint(model);
    const latestParse = graphParseCoverage(v, eng) || { complete: true };
    const latestDone = !!latestParse.complete;
    const need = ctx._liveFp == null || latestFp !== ctx._liveFp
      || (cur.mode === 'global' && ctx._liveParseComplete === false && latestDone);
    if (!need) {
      ctx._liveParseComplete = latestDone;
      refreshGraphLoadingChrome(v, win, ctx, eng);
      return;
    }
    performLiveRemount(v, win, ctx, eng, latestFp, latestDone);
  };
  if (immediate) {
    clearLiveRemountTimer(ctx);
    run();
    return;
  }
  if (ctx._liveRemountTimer) return;
  ctx._liveRemountTimer = setTimeout(run, LIVE_REMOUNT_MS);
}

function syncGraphLive(view, win, ctx, { forceRemount = false } = {}) {
  if (!view || !win || !ctx) return;
  const cur = navCurrent(win._graphNav);
  if (!cur) return;
  ...
  if (bundle) {
    ...
    if (bundle.live) bundle.live.view = view;
  }

  // Chrome is cheap — always refresh immediately so parse % stays live.
  const parseCoverage = refreshGraphLoadingChrome(view, win, ctx, engine);
  ...
  const shouldRemount = forceRemount || ctx._liveFp == null || fp !== ctx._liveFp
    || (cur.mode === 'global' && ctx._liveParseComplete === false && parseDone);
  if (!shouldRemount) {
    ctx._liveParseComplete = parseDone;
    return;
  }
  // Full remount (loseContext + force layout) is expensive — coalesce rapid
  // parse-growth ticks so tab switches / live indexing can't lock the UI.
  scheduleLiveRemount(view, win, ctx, engine, fp, parseDone, {
    immediate: forceRemount,
  });
}
...
function openGraphSettingsDialog() {
  ...
  const g = typeof window !== 'undefined' ? window : self;
  if (graphSettingsDialog && g.Dialog) g.Dialog.openDialog(graphSettingsDialog);
}
...
function viewMetaFor(engine, model, mode, depth = 1, { view } = {}) {
  ...
  if (mode === 'global' && model) {
    ...
    if (view) {
      const cov = graphParseCoverage(view, engine);
      if (cov && !cov.complete) meta.parsePercent = cov.percent;
    }
  }
  return meta;
}
...
function buildGraphToolbar({ sidebar } = {}) {
  ...
  el.append(searchWrap, localBtn, globalBtn, resetBtn, zoomOutBtn, zoomInBtn);
  ...
  function reflectScopeBtn(isGlobal) {
    inGlobal = !!isGlobal;
    globalBtn.hidden = inGlobal;
  }

  function reflectActions() {
    const on = !!ctrlRef()?.getFocusedNode?.()?.node || selectedIdRef?.() != null;
  ...
  return {
    ...
    wire(getCtrl, { onReset, onExplore, onGlobal, getSelectedId } = {}) {
      ctrlRef = getCtrl;
      selectedIdRef = getSelectedId || (() => null);
      reflectActions();
...
function mountGraph3DRenderer(view, engine, ctx) {
  ...
  // Node clicks only focus within the current graph. Local/global mode changes
  // go through the toolbar buttons (onExplore / onGlobal), never via onDrill.
  const graphOpts = {
...
function navigateGraph(view, engine, win, ctx, entry, { push = true, restoreIndex = null } = {}) {
  ...
  win._graph3d = null;
  // dispose() loses the WebGL context; getContext on the same canvas returns the
  // dead context object (not null). Replace the canvas so the next mount gets a fresh one.
  const freshCanvas = ctx.canvas.cloneNode(false);
  ctx.canvas.replaceWith(freshCanvas);
  ctx.canvas = freshCanvas;
...
function open3DGraph(view, engine, key, model, titleNode, {
  ...
  function startGraphLive() {
    ...
    ctx._liveParseComplete = view ? !!(graphParseCoverage(view, engine)?.complete) : true;
    const liveEntry = {
      view,
      sync: () => {
        const v = bundleView(win);
        if (!v) return;
        syncGraphLive(v, win, ctx);
      },
    };
    graphBundle.live = liveEntry;
    unregisterGraphLive = registerGraphLive(liveEntry);
    if (view && !ctx._liveParseComplete) {
      queueMicrotask(() => syncGraphLive(bundleView(win) || view, win, ctx));
    }
  }
  ...
  function setGraphFollow(on) {
    ...
    if (ctx.followEditor) {
      unregisterGraphFollow = registerEditorFollow({
        get view() { return bundleView(win); },
        isActive: () => ctx.followEditor,
        sync: () => {
          const v = bundleView(win);
          const eng = bundleEngine(win);
          if (v && eng) syncFollowFromEditor(v, eng, win, ctx);
        },
      });
      const v = bundleView(win);
      const eng = bundleEngine(win);
  ...
  const onKey = (ev) => {
    ...
    if (ev.key === '/' && !ev.metaKey && !ev.ctrlKey) { ev.preventDefault(); toolbar.focusSearch?.(); }
      ...
      ev.preventDefault();
      if (v && eng) navBackGraph(v, eng, win, ctx);
    } else if (ev.altKey && ev.key === 'ArrowRight') {
      ev.preventDefault();
      if (v && eng) navForwardGraph(v, eng, win, ctx);
    }     else if (ev.key === 'Enter') {
      const f = ctrl?.getFocusedNode?.();
  ...
  toolbar.wire(() => win._graph3d, {
    ...
    onExplore: (node) => {
      ...
      drillGraph(v, eng, win, ctx, node.id);
    },
    onGlobal: () => {
      const v = bundleView(win);
      const eng = bundleEngine(win);
      if (v && eng) navigateToGlobalGraph(v, eng, win, ctx);
    },
  });
...
function rebindGraphWindowsToActiveEditor(view) {
  ...
  for (const win of openGraphWindows.values()) {
    ...
    // Remount only when the model fingerprint actually changes; chrome (parse %)
    // always refreshes. Forcing a remount on every tab switch was locking the UI.
