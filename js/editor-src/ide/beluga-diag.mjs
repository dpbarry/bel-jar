...
export function belugaOutputLooksLikeFailure(raw) {
  ...
  // Beluga Unbound_identifier printer: "Identifier X is unbound." (no Error:)
  if (/\bis unbound\b/i.test(text)) return true;
...
// A failed check that produced no locatable diagnostic must still surface ?
// a red squiggle beats a silent green. Point it at the named culprit token when
...
function firstFailureMessage(raw) {
  const text = stripAnsi(raw);
  const err = text.match(/^Error:\s*(.+)$/im);
  if (err) return err[1].trim();
  if (!belugaOutputLooksLikeFailure(text)) return null;
  for (const line of text.split('\n')) {
    const t = line.trim();
    if (!t) continue;
    if (/^File\s+"/i.test(t)) continue;
    if (/^##\s/.test(t)) continue;
    if (/^Raised at|^Called from|^Re-raised|^Backtrace/i.test(t)) break;
    return t.replace(/^(Error|Warning):\s*/i, '');
  }
  return null;
}

// Beluga's Unbound_identifier printer emits bare "Identifier X is unbound." with
// no File/Error wrapper (unlike Located_exception). When we recover a location,
// re-seat it in the normal Beluga report shape used by Run / hover.
export function formatBelugaErrorReport(detail, loc = {}) {
  const body = polishBelugaMessage(String(detail ?? '').replace(/^(Error|Warning):\s*/i, ''));
  const file = loc.file != null && String(loc.file).trim() ? String(loc.file).trim() : null;
  const line = Number(loc.line);
  const column = Number(loc.column);
  const parts = [];
  if (file && line >= 1) {
    parts.push(`File "${file}", line ${line}${column >= 1 ? `, column ${column}` : ''}`);
  }
  parts.push(body ? `Error: ${body}` : 'Error:');
  return parts.join('\n');
}

export function fallbackDiagnostic(raw, doc, { fileName = null } = {}) {
  const short = firstFailureMessage(raw);
  ...
  const column = span.from - line.from + 1;
  const message = formatBelugaErrorReport(polished, {
    file: fileName || 'input.bel',
    line: line.number,
    column,
  });
