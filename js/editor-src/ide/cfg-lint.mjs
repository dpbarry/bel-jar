...
import { isCfgEntryToken, isCfgSourceEntry } from '../project-paths.mjs';
import { linter } from '@codemirror/lint';
import { dirOf, joinPath } from '../semantic/development.mjs';
import { analyzeSuite, findingMessage } from './suite-lint.mjs';
import { lintLinterOptions } from './syntax-lint.mjs';
...
// Headless variant for file-health consumers (explorer dots, problems mode):
// plain text + project path, live registry supplies names and content.
export function cfgFileDiagnostics(text, cfgPath) {
  return cfgDiagnosticsFor(String(text ?? ''), cfgPath, projectFileNames(), makeRegistryGetText());
}
