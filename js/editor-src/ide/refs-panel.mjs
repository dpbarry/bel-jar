...
import { navInfoAt, termRangeAt, crossFileDefinitionAt, peekRange } from './ide-actions.mjs';
import { groupReferencesFor, usesOf, referenceGroupFilesFor } from '../semantic/project-prelude.mjs';
...
// Persist active file — drives cross-file reference indexing (module group).
...
export function findReferences(view, pos) {
  ...
  const gPeek = g;
  g.SuppressRefPeek = true;
  g.BelJarSuppressRefPeek = g.SuppressRefPeek
  g.Menu.openContext({
    x, y,
    side: 'bottom',
    align: 'start',
