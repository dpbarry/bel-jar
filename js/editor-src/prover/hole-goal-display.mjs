...
import { approximateHoleGoal } from './prover-orchestrator.mjs';
...
export function resolveHoleGoalDisplay({
  ...
  // Outside active development: still show a syntactic approx from the decl
  // header (no Beluga). Nested holes without a recoverable header stay empty.
  if (approximateGoal) {
    return { goal: approximateGoal, state: 'approximate', loadingLive: false };
  }
...
export function buildHoleDisplayRows({
  ...
  for (const h of syntacticHoles || []) {
    ...
    const approx = approximateHoleGoal(fileText, h.line, h.col);
...
export function resolveHoleGoalForPosition({
  ...
  const approx = approximateHoleGoal(fileText, line, col);
...
export function resolveGoalStateNearPos(engine, doc, pos, ctx) {
  ...
  for (const h of engine.getHoles()) {
    ...
    const approx = approximateHoleGoal(ctx.fileText, h.line, h.col);
    return resolveHoleGoalDisplay({
