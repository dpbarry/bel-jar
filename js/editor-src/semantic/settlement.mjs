...
export function createSettlement({
  ...
  async function settleNow(syntaxSnap, gen, traceOptions = {}) {
    ...
    function ensurePreludeBlocks() {
      ...
      const ptree = parser.parse(String(rawPrelude.code));
      const { blocks, blockAt } = computeLintBlocks(ptree, pdoc);
      preludeBlocks = { blocks, blockAt, doc: pdoc, tree: ptree };
    ...
    // An unbound-identifier reported in the prelude whose every definition lives
    // in an ALREADY-MASKED prelude block is induced by the masking itself — a
    // cascade of the real error, not a second independent fault. It still gets
    // masked (so the loop advances) but must never be reported or counted.
    function isInducedPreludeIssue(iss) {
      if (!maskedPrelude.size) return false;
      const culprit = namedCulprit(iss?.message || '');
      if (!culprit) return false;
      const pb = ensurePreludeBlocks();
      if (!pb) return false;
      const entries = walkTree(pb.tree, pb.doc).defMap.get(culprit);
      if (!entries || !entries.length) return false;
      let sawDef = false;
      for (const entry of entries) {
        const hit = pb.blockAt(entry.ident.from);
        if (!hit) continue;
        sawDef = true;
        if (!maskedPrelude.has(hit.index)) return false;
      }
      return sawDef;
    }
    ...
    function processCheckerOutput(rawOutput, ok) {
      ...
      let preludeIssues = shifted.preludeIssues || [];
    ...
    try {
      ...
      for (let pass = 0; pass < passLimit; pass += 1) {
        ...
        // Induced test runs BEFORE masking this pass's issues: it must compare
        // against blocks masked in EARLIER passes only.
        for (const iss of preludeIssues) {
          if (isInducedPreludeIssue(iss)) continue;
