(() => {
  ...
  function suggestDefaultFileName(parentDir2, files) {
    var nc = NC();
    var base = "untitled.bel";
    var full = joinPath(parentDir2, base);
  ...
  function suggestDefaultFolderName(parentDir2, files, emptyFolders) {
    var nc = NC();
    var full = joinPath(parentDir2, "untitled");
    ...
    while (folderPathExists(files, emptyFolders, full)) {
      n += 1;
      full = joinPath(parentDir2, "untitled-" + n);
    }
    return full;
  }
  ...
  global.ExplorerInlineName = {
    ...
    suggestDefaultFolderName,
    validateFileCommit,
    validateFolderCommit,
    resolveCreateParentFromRow,
    resolveCreateParentDir
  };
  global.BelJarExplorerInlineName = global.ExplorerInlineName;
    ...
    for (var i = 0; i < files.length; i++) {
      ...
      for (var j = 0; j < parts.length - 1; j++) {
        ...
        if (!node.folders.has(parts[j])) {
          node.folders.set(parts[j], { path, folders: /* @__PURE__ */ new Map(), files: [] });
        }
    ...
    for (var k = 0; k < empty.length; k++) {
      ...
      for (var m = 0; m < segs.length; m++) {
        ...
        if (!node2.folders.has(segs[m])) {
          node2.folders.set(segs[m], { path: path2, folders: /* @__PURE__ */ new Map(), files: [] });
        }
  ...
  function resolveCreateParentFromRow2(row) {
    var IL = global2.ExplorerInlineName;
  ...
  function dragRootsFromSelection(fileIds, folderPaths, existingFiles) {
    ...
    for (var i = 0; i < (fileIds || []).length; i++) {
      ...
      for (var j = 0; j < existingFiles.length; j++) {
        if (existingFiles[j].id === fileIds[i]) {
          file = existingFiles[j];
          break;
        }
      }
      if (!file) continue;
      ...
      for (var k = 0; k < fps.length; k++) {
        if (isPathUnderFolder(file.name, fps[k])) {
          covered = true;
          break;
        }
      }
    ...
    for (var f = 0; f < fps.length; f++) {
      ...
      for (var g = 0; g < fps.length; g++) {
        if (fps[g] !== fp && isPathUnderFolder(fp, fps[g])) {
          nested = true;
          break;
        }
      }
  ...
  function selectionDragCapability(fileIds, folderPaths, existingFiles, emptyFolders) {
    ...
    for (var ri = 0; ri < roots.fileIds.length; ri++) {
      ...
      for (var rj = 0; rj < existingFiles.length; rj++) {
        if (existingFiles[rj].id === roots.fileIds[ri]) {
          fileR = existingFiles[rj];
          break;
        }
      }
  ...
  function sameParentFileIdsForDrag(fileIds, rowKey, existingFiles) {
    ...
    var dragged = null;
    for (var i = 0; i < existingFiles.length; i++) {
      if (existingFiles[i].id === rowKey.key) {
        dragged = existingFiles[i];
        break;
      }
    }
  ...
  function init(opts) {
    ...
    function collapseSubtree(folderPath) {
      ...
      if (!folderPath) {
        ...
        collectSubtreeFolderPaths(model, folderPath).forEach(function(p) {
          collapsed.add(p);
        });
      }
      scheduleSave();
    ...
    function withLeadingItems(extra, base) {
      if (extra && extra.length) {
        return endsWithSeparator(extra) ? extra.concat(base) : extra.concat([{ type: "separator" }], base);
      }
      return base;
    }
    function folderContextItems(folderPath) {
      var extra = typeof opts.getFolderContextItems === "function" ? opts.getFolderContextItems(folderPath) : null;
      return extra && extra.length ? extra : [];
    }
    function treeBackgroundItems() {
      var base = [
        { label: "Expand all", onSelect: function() {
          expandSubtree("");
        } },
        { label: "Collapse all", onSelect: function() {
          collapseSubtree("");
        } }
      ];
      var extra = typeof opts.getBackgroundContextItems === "function" ? opts.getBackgroundContextItems() : null;
    ...
    function renderNode(treeEl, node, depth, zonePath) {
      ...
      node.folders.forEach(function(folder, name) {
        ...
        row.type = "button";
        row.className = "explorer-folder-item" + (isCollapsed ? " is-collapsed" : "") + (editing ? " is-renaming" : "") + (selectedFolders.has(folder.path) ? " is-selected" : "");
      ...
      for (var i = 0; i < node.files.length; i++) {
        ...
        btn.className = "explorer-file-item" + (file.id === activeId ? " is-active" : "") + (selectedFiles.has(file.id) ? " is-selected" : "") + (isCfg ? " explorer-file-item--cfg" : "") + (isElf ? " explorer-file-item--elf" : "") + (suiteMeta && (suiteMeta.role === "head" || suiteMeta.role === "solo") ? " explorer-file-item--suite-cfg" : "") + (suiteMeta && (suiteMeta.role === "mid" || suiteMeta.role === "tail") ? " explorer-file-item--suite" : "") + (suiteMeta && suiteMeta.suiteIndex > 0 && suiteMeta.role === "head" ? " explorer-suite-block-sep" : "") + (editingFile ? " is-renaming" : "");
        ...
        if (editingFile) {
          ...
          btn.addEventListener("click", /* @__PURE__ */ (function(fileRow) {
            return function(ev) {
              handleRowClick(ev, fileRow);
            };
          })(btn));
        }
    ...
    function refresh() {
      ...
      if (inlineInputEl) {
        ...
        var sel = prevFocus.indexOf("/") !== -1 || !prevFocus.includes(".") ? '[data-folder-path="' + prevFocus + '"]' : '[data-file-id="' + prevFocus + '"]';
    ...
    if (typeof global2.Menu !== "undefined") {
      global2.Menu.bindContextMenu(container, function(e) {
    ...
    function resolveDrop(clientX, clientY) {
      ...
      if (typeof document.elementsFromPoint === "function") {
        ...
        for (var si = 0; si < stack.length; si++) {
          var candidate = stack[si].closest && stack[si].closest(".explorer-folder-item, .explorer-file-item");
          if (candidate && container.contains(candidate) && !candidate.classList.contains("is-dragging")) {
            row = candidate;
            break;
          }
        }
    ...
    if (typeof global2.TreeDnD !== "undefined" && typeof opts.onDrop === "function") {
      dndDetach = global2.TreeDnD.attach(container, {
    ...
    return {
      ...
      clearSelection,
      setSelection,
      shouldKeepSelectionOnOpen: function() {
        return suppressClearSelection;
      },
  ...
  global2.Explorer = {
    ...
    rootZoneTopFromLastRow,
    resolveCreateParentFromRow: resolveCreateParentFromRow2,
    resolveCreateParentDir: resolveCreateParentDir2,
    parentDirFromName,
    rowKeyFromEl,
    rangeSelectVisibleRows,
    toggleCtrlSelection,
    selectionDragCapability,
    sameParentFileIdsForDrag,
    init
  };
  global2.BelJarExplorer = global2.Explorer;
})();
