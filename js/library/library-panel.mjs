...
var CHEVRON_SVG =
  ...
  function init(opts) {
    ...
    /** True when the active file lives under a folder (not project root / no file). */
    ...
    function refreshSuites() {
      ...
      } else if (global.LibrarySuites) {
        suitesCache = global.LibrarySuites.listActiveSuites({
    ...
    function openLibraryPreview(previewOpts) {
      if (!global.LibraryPreview || typeof global.LibraryPreview.open !== 'function') return;
      global.LibraryPreview.open({
    ...
    function resolveBulkPlan(incoming, existingFiles) {
      var NC = global.NameConflicts;
      var CD = global.ConflictDialog;
      if (!NC) {
      ...
      openLibraryMenu(anchor, {
        ...
        items: [
          { label: 'Insert at root', onSelect: function () { runFolderInsert(folder, 'root'); } },
          {
            ...
            onSelect: function () { runFolderInsert(folder, 'under'); },
          },
    ...
    if (searchEl && searchWrap && global.HeaderSearch) {
      global.HeaderSearch.init({
        host: searchWrap,
        input: searchEl,
  ...
  global.Library = { init: init };
  global.BelJarLibrary = global.Library
