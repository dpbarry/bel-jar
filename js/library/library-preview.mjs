...
var CHEVRON_SVG =
  ...
  function open(opts) {
    opts = opts || {};
    if (!opts.scopeFolder || typeof global.Dialog === 'undefined') return;

    if (activeDialog && activeDialog.open) {
      global.Dialog.requestDialogClose(activeDialog);
      activeDialog = null;
    }
  ...
  function close() {
    if (activeDialog && global.Dialog) {
      global.Dialog.requestDialogClose(activeDialog);
    }
  }

  global.LibraryPreview = { open: open, close: close };
  global.BelJarLibraryPreview = global.LibraryPreview
