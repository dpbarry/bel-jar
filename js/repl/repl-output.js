...
(() => {
  ...
  function normBelugaRaw(s) {
    return global.BelugaText ? global.BelugaText.normalizeBelugaRaw(s) : String(s != null ? s : "").replace(/\r\n/g, "\n");
  }
  ...
  function attachQuerySourceLines(segs, sourceLines) {
    ...
    return segs.map(function(seg) {
      ...
      return {
        type: "query",
        query: Object.assign({}, seg.query, { sourceLine })
      };
    });
  }
  ...
  function queryDisplayRows(bindings) {
    return global.BelugaText && global.BelugaText.prettifyQueryBindings ? global.BelugaText.prettifyQueryBindings(bindings) : bindings || [];
  }
  ...
  function appendQueryFormatted(raw) {
    ...
    if (!trimmed) {
      appendRichMsg("success", "Query completed.", raw);
      return;
    }
  ...
  function segmentRunOutput(text) {
    ...
    var otherLines = [];
    function flushOther() {
      var filtered = otherLines.filter(function(l) {
        return !/^Done\.?\s*$/.test(l.trim()) && !/^\s*;\s*$/.test(l.trim());
      }).join("\n").trim();
      ...
      if (/^##\s/.test(trimmed)) {
        ...
        if (/##\s*Holes:/i.test(trText)) {
          ...
          for (var k = 0; k < allTr.length; k++) {
            if (/##\s*Holes:/i.test(allTr[k].trim())) {
              holeStart = k;
              break;
            }
          }
  ...
  function appendBelugaResponse(raw, verb) {
    if (typeof BelugaRun !== "undefined" && typeof ProjectSource !== "undefined" && typeof BelugaRun.getProjectSpans === "function") {
  ...
  function classifyRunOtherKind(text) {
    ...
    var isTypeReconStatus = segLines.length > 0 && segLines.every(function(line) {
