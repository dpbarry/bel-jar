(() => {
  ...
  function findSurfaceSearchInput(root) {
    ...
    const el2 = root.querySelector(SURFACE_SEARCH_SELECTOR);
    if (!el2 || el2.disabled) return null;
    return el2;
  }
  function capturingSearchInput() {
    if (typeof document === "undefined") return null;
    const open3 = document.querySelectorAll("dialog." + DIALOG_ROOT_CLASS + "[open]:not(.is-leaving)");
    for (let i = open3.length - 1; i >= 0; i--) {
