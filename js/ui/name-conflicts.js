(() => {
  ...
  function computeMoveTargets(existingFiles, payload, dropTarget, getText) {
    ...
    if (payload.kind === "file") {
      var file = null;
      for (var i = 0; i < existingFiles.length; i++) {
        if (existingFiles[i].id === payload.fileId) {
    ...
    if (payload.kind === "files") {
      ...
      for (var fi = 0; fi < ids.length; fi++) {
        ...
        for (var im = 0; im < existingFiles.length; im++) {
          if (existingFiles[im].id === ids[fi]) {
            fileM = existingFiles[im];
            break;
          }
        }
        if (!fileM) continue;
    ...
    if (payload.kind === "selection") {
      ...
      for (var sfi = 0; sfi < selFileIds.length; sfi++) {
        ...
        for (var si = 0; si < existingFiles.length; si++) {
          if (existingFiles[si].id === selFileIds[sfi]) {
            fileS = existingFiles[si];
            break;
          }
        }
        if (!fileS) continue;
  ...
  function canDropMove(payload, dropTarget, existingFiles, emptyFolders) {
    ...
    if (payload.kind === "folder") {
      ...
      var moves = computeMoveTargets(existingFiles, payload, dropTarget, function() {
        return "";
      });
    ...
    if (payload.kind === "file") {
      var movesF = computeMoveTargets(existingFiles, payload, dropTarget, function() {
        return "";
      });
    ...
    if (payload.kind === "files") {
      ...
      for (var ci = 0; ci < ids.length; ci++) {
        ...
        for (var cj = 0; cj < existingFiles.length; cj++) {
          if (existingFiles[cj].id === ids[ci]) {
            fileC = existingFiles[cj];
            break;
          }
        }
      ...
      var movesFs = computeMoveTargets(existingFiles, payload, dropTarget, function() {
        return "";
      });
    ...
    if (payload.kind === "selection") {
      ...
      var movesSel = computeMoveTargets(existingFiles, payload, dropTarget, function() {
        return "";
      });
  ...
  function longestCommonDirectoryPrefix(paths) {
    ...
    var minLen = Math.min.apply(null, split.map(function(s) {
      return s.length;
    }));
    var parts = [];
  ...
  function folderMoveBatchRoot(moves, existingFiles) {
    ...
    var fromPrefix = longestCommonDirectoryPrefix(moves.map(function(m2) {
      return m2.from;
    }));
    ...
    var toPrefix = longestCommonDirectoryPrefix(moves.map(function(m2) {
      return m2.to;
    }));
  ...
  function detectMoveConflicts(existingFiles, moves, opts) {
    ...
    var remaining = existingFiles.filter(function(f) {
      return !sourceIds[f.id];
    });
  ...
  function applyMoveResolutions(existingFiles, moves, conflicts, resolutions) {
    ...
    for (var i = 0; i < conflicts.length; i++) {
      ...
      if (r.action === "replace") {
        if (c.kind === "folder") {
          var related = c.moves || moves.filter(function(m2) {
            return m2.from.indexOf(c.path + "/") === 0 || m2.from === c.path;
          });
          var folderRenames = [];
          var deleteIds = [];
          var moveIdSet = {};
          ...
          replaceFolder.push({ prefix: c.path, deleteIds, renames: folderRenames });
        } else if (c.moveId) {
          var targetFile = null;
          for (var t = 0; t < existingFiles.length; t++) {
            if (existingFiles[t].name === c.path) {
              targetFile = existingFiles[t];
              break;
            }
          }
          if (targetFile) {
      ...
      if (r.action === "rename") {
        ...
        if (c.kind === "folder") {
          var rel = c.moves || moves.filter(function(m2) {
            return m2.from.indexOf(c.path + "/") === 0;
          });
      ...
      if (Object.prototype.hasOwnProperty.call(renames, move.id)) {
        ...
        for (var rf = 0; rf < replaceFolder.length; rf++) {
          for (var rr = 0; rr < replaceFolder[rf].renames.length; rr++) {
            if (replaceFolder[rf].renames[rr].id === move.id) {
              handled = true;
              break;
            }
          }
        ...
        for (var rp = 0; rp < replaces.length; rp++) {
          if (replaces[rp].deleteId === move.id) {
            handled = true;
            break;
          }
        }
  ...
  function detectUploadConflicts(existingFiles, incomingEntries, opts) {
    ...
    var existingPaths = existingFiles.map(function(f) {
      return f.name;
    });
    var existingSet = {};
    var pathToId = {};
    ...
    for (var folderPath in folderCandidates) {
      ...
      var hasCollision = incomingUnder.some(function(e) {
        return existingSet[e.name];
      });
  ...
  function applyResolutions(existingFiles, incomingEntries, conflicts, resolutions) {
    ...
    for (var rf = 0; rf < replaceFolders.length; rf++) {
      ...
      plan.replaceFolder.push({
        ...
        deleteIds,
        entries: folder.entries.map(function(e) {
    ...
    for (var h = 0; h < incomingEntries.length; h++) {
      ...
      var targetName = Object.prototype.hasOwnProperty.call(renames, ent.name) ? renames[ent.name] : ent.name;
      if (pathToFile[targetName]) continue;
      plan.create.push({ name: targetName, text: ent.text });
    }
    return plan;
  }
  global.NameConflicts = {
    parentDir,
    baseName,
    joinPath,
    nameConflict,
    suggestNewPath,
    splitNumberedStem,
    folderPathsFromFilePaths,
    occupiedFolderPaths,
    isDescendantPath,
    filesUnderPrefix,
    emptyFoldersUnderPrefix,
    detectUploadConflicts,
    applyResolutions,
    uploadFolderBatchRoots,
    folderMoveBatchRoot,
    computeMoveTargets,
    computeEmptyFolderMoves,
    canDropMove,
    detectMoveConflicts,
    applyMoveResolutions
  };
  global.BelJarNameConflicts = global.NameConflicts;
})();
