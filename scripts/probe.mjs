...
try {
  ...
  const pal = await page.evaluate(() => ({
    rows: document.querySelectorAll('.jar-palette-item').length,
    sections: [...document.querySelectorAll('.jar-palette-section')].map((e) => e.textContent),
    first: [...document.querySelectorAll('.jar-palette-item-title')].slice(0, 3).map((e) => e.textContent),
  ...
  const gated = await page.evaluate(() => {
    const titles = [...document.querySelectorAll('.jar-palette-item-title')].map((e) => e.textContent);
  ...
  const sheet = await page.evaluate(() => ({
    rows: document.querySelectorAll('.jar-kb__row').length,
    sections: [...document.querySelectorAll('.jar-kb__section')].map((e) => e.textContent),
    hasFilter: !!document.querySelector('.jar-kb__filter-input'),
    count: (document.querySelector('.jar-kb__filter-count') || {}).textContent || '',
    unbound: [...document.querySelectorAll('.jar-kb__chord.is-empty')].length,
  }));
  console.log('  sheet:', JSON.stringify(sheet));
  check(sheet.rows === 147, `sheet renders every bindable command (got ${sheet.rows})`);
  check(
  ...
  const scrollports = await page.evaluate(() => {
    const panel = document.querySelector('.jar-settings__panel[data-category="keybindings"]');
    const body = panel.querySelector('.jar-settings__panel-body');
    ...
    return {
      ...
      filterSticky: getComputedStyle(panel.querySelector('.jar-kb__filter')).position,
    };
  });
  ...
  check(/147 commands · 20 bound/.test(sheet.count), 'filter count reads right', sheet.count);
  check(sheet.unbound === 127, `unbound rows render as empty chords (got ${sheet.unbound})`);
  await page.screenshot({ path: path.join(outDir, 'keybindings-sheet.png') });

  // ── filtering ───────────────────────────────────────────────────────────────
  await page.type('.jar-kb__filter-input', 'hole');
  await new Promise((r) => setTimeout(r, 250));
  const filtered = await page.evaluate(() => {
    const rows = [...document.querySelectorAll('.jar-kb__row')];
    const heads = [...document.querySelectorAll('.jar-kb__section')];
    return {
      visible: rows.filter((r) => !r.hidden).map((r) => r.dataset.commandId),
      visibleHeads: heads.filter((h) => !h.hidden).map((h) => h.textContent),
      count: (document.querySelector('.jar-kb__filter-count') || {}).textContent || '',
      noResultsHidden: document.querySelector('.jar-kb__noresults').hidden,
    };
  });
  console.log('  filtered:', JSON.stringify(filtered));
  check(filtered.visible.length > 0 && filtered.visible.length < 10, `"hole" narrows the sheet (${filtered.visible.length} rows)`);
  const filteredTitles = await page.evaluate(() => [...document.querySelectorAll('.jar-kb__row')]
    .filter((r) => !r.hidden).map((r) => r.querySelector('.jar-kb__title').textContent));
  check(
  ...
  await page.evaluate(() => {
    const el = document.querySelector('.jar-kb__filter-input');
    el.value = 'zzzznope';
  ...
  const empty = await page.evaluate(() => ({
    noResultsHidden: document.querySelector('.jar-kb__noresults').hidden,
    visible: [...document.querySelectorAll('.jar-kb__row')].filter((r) => !r.hidden).length,
  }));
  ...
  const live = await page.evaluate(() => {
    const bar = document.querySelector('.jar-strip');
    const segs = [...bar.querySelectorAll('.jar-strip__seg')];
    return {
      keys: segs.map((e) => (e.className.match(/seg--([\w-]+)/) || [])[1]),
      texts: segs.map((e) => e.textContent.trim()),
      hasSpacer: !!bar.querySelector('.jar-strip__spacer'),
    };
  });
  ...
  const survives = await page.evaluate(async () => {
    const before = document.querySelector('.jar-strip .ide-status-dot');
    before.setAttribute('data-live-state', 'checking');
    StatusStrip.setEditorState({ line: 999, col: 12 });
    await new Promise((r) => requestAnimationFrame(() => requestAnimationFrame(r)));
    const after = document.querySelector('.jar-strip .ide-status-dot');
  ...
  const goalRender = await page.evaluate(async () => {
    StatusStrip.setEditorState({ inHole: true, goal: "[ |- dual A A'] -> [ |- dual A' A]" });
    await new Promise((r) => requestAnimationFrame(() => requestAnimationFrame(r)));
    const seg = document.querySelector('.jar-strip__seg--goal');
    if (!seg) return { missing: true };
    const label = seg.querySelector('.jar-strip__label');
    const cs = getComputedStyle(seg);
    return {
      text: seg.textContent,
      mark: (seg.querySelector('.jar-strip__mark') || {}).textContent,
      spans: label.querySelectorAll('span[class*="jar-hl"]').length,
  ...
  // ⛔ THE THIRD STATE. Being in a hole and knowing its goal are different
  // facts, and while they were one string the bar reported a hole whose goal
  // was still being computed as NO HOLE AT ALL: you stood on a fresh `?` and it
  // said nothing, with no way to say the honest thing.
  const goalStates = await page.evaluate(async () => {
    const settle = () => new Promise((r) => requestAnimationFrame(() => requestAnimationFrame(r)));
    const read = () => {
      const seg = document.querySelector('.jar-strip__seg--goal');
      const holes = document.querySelector('.jar-strip__seg--holes');
      if (!seg) return { missing: true, holes: holes ? holes.textContent : null };
      return {
        text: seg.textContent,
        tag: seg.tagName,
        pending: seg.classList.contains('is-pending'),
        wash: getComputedStyle(seg).backgroundColor,
        highlighted: seg.querySelectorAll('span[class*="jar-hl"]').length,
        holes: holes ? holes.textContent : null,
      };
    };
    StatusStrip.setEditorState({ holes: 3, inHole: true, goal: '', goalPending: true });
    await settle();
    const computing = read();
    StatusStrip.setEditorState({ goalPending: false });
    await settle();
    const settled = read();
    StatusStrip.setEditorState({ inHole: false, goal: '' });
    await settle();
    const away = read();
    StatusStrip.setEditorState({ holes: 0, inHole: false, goal: '', goalPending: false });
    await settle();
    return { computing, settled, away };
  });
  console.log('  goal states:', JSON.stringify(goalStates));
  check(/Computing/.test(goalStates.computing.text || ''),
    'a hole whose goal is still being computed says so', goalStates.computing.text);
  check(goalStates.computing.pending && goalStates.computing.tag === 'SPAN',
    'the placeholder is a label, not a button that cannot work yet',
    JSON.stringify(goalStates.computing));
  check(goalStates.computing.wash === goalRender.wash,
    'and keeps the hole wash, so the chip does not appear and jump when the goal lands',
    goalStates.computing.wash + ' vs ' + goalRender.wash);
  check(goalStates.computing.highlighted === 0,
    'a placeholder is not syntax-highlighted — there is no syntax yet');
  check(/\+2 more/.test(goalStates.computing.holes || ''),
    'the hole count knows you are standing in one even before the goal exists',
    goalStates.computing.holes);
  // ⛔ A spinner that never resolves is a lie told slowly. A hole inside a
  // declaration that failed to check never gets a goal.
  check(/No goal/.test(goalStates.settled.text || ''),
    'settled with no goal stops saying Computing', goalStates.settled.text);
  check(goalStates.away.missing === true,
    'and out of a hole there is no chip at all', JSON.stringify(goalStates.away));
  ...
  const clicks = await page.evaluate(async () => {
    ...
    const holeSeg = document.querySelector('.jar-strip__seg--holes');
  ...
  const line = await page.evaluate(async () => {
    ...
    await new Promise((r) => requestAnimationFrame(r));
    const el = document.querySelector('.jar-cmdline');
    const input = document.querySelector('.jar-cmdline__input');
    return {
      open: StatusStrip.isCommandLineOpen(),
      visible: el ? !el.hidden : false,
      focused: document.activeElement === input,
      segmentsHidden: getComputedStyle(document.querySelector('.jar-strip__segments')).display === 'none',
      // The facts that were on screen a moment ago must still be on screen.
      keeps: ['keymap', 'position'].filter((k) => document.querySelector('.jar-strip__seg--' + k)),
      inZone: !!document.querySelector('.jar-strip__command .jar-cmdline'),
      listed: document.querySelectorAll('.jar-cmdline__item').length,
      barH: Math.round(document.querySelector('.jar-strip').getBoundingClientRect().height),
    };
  });
  ...
  const typed = await page.evaluate(async () => {
    ...
    return {
      ghost: document.querySelector('.jar-cmdline__ghost').textContent,
      first: (document.querySelector('.jar-cmdline__item-name') || {}).textContent,
      ...
      barTop: Math.round(document.querySelector('.jar-strip').getBoundingClientRect().top),
      listAbove: list && !list.hidden ? list.getBoundingClientRect().bottom <= document.querySelector('.jar-strip').getBoundingClientRect().top + 1 : false,
    };
  });
  ...
  const unknown = await page.evaluate(async () => {
    ...
    input.dispatchEvent(new KeyboardEvent('keydown', { key: 'Enter', bubbles: true }));
    await new Promise((r) => setTimeout(r, 250));
    const msg = document.querySelector('.jar-strip__message');
  ...
  const isearch = await page.evaluate(async () => {
    ...
    const input = document.querySelector('.jar-cmdline__input');
    const prompt = document.querySelector('.jar-cmdline__prompt').textContent;
    input.value = 'line 150';
    input.dispatchEvent(new Event('input', { bubbles: true }));
    await new Promise((r) => setTimeout(r, 250));
    const count = document.querySelector('.jar-cmdline__count').textContent;
    ...
    input.dispatchEvent(new Event('input', { bubbles: true }));
    await new Promise((r) => setTimeout(r, 200));
    const missCount = document.querySelector('.jar-cmdline__count').textContent;
  ...
  const msg = await page.evaluate(async () => {
    ...
    const bar = document.querySelector('.jar-strip');
    const before = {
      pos: boxOf('.jar-strip__seg--position'),
      checker: boxOf('.jar-strip__seg--checker'),
      barW: bar ? Math.round(bar.getBoundingClientRect().width) : 0,
      segW: Math.round((document.querySelector('.jar-strip__segments') || { getBoundingClientRect: () => ({ width: 0 }) }).getBoundingClientRect().width),
    };

    StatusStrip.setMessage('Word wrap off');
    await new Promise((r) => setTimeout(r, 60));
    const node = document.querySelector('.jar-strip__message');
    const mid = {
      text: node ? node.textContent : '',
      visible: node ? node.classList.contains('is-visible') : false,
      opacity: node ? getComputedStyle(node).opacity : '',
      segmentsStillThere: document.querySelectorAll('.jar-strip__seg').length,
      onRight: node && document.querySelector('.jar-strip__seg--position')
        ? node.getBoundingClientRect().left > document.querySelector('.jar-strip__seg--position').getBoundingClientRect().left
        : false,
      rects: { before, after: { pos: boxOf('.jar-strip__seg--position'), checker: boxOf('.jar-strip__seg--checker') } },
      noShift: before.pos === boxOf('.jar-strip__seg--position')
        && before.checker === boxOf('.jar-strip__seg--checker'),
      afterSpacer: node ? (node.previousElementSibling || {}).className === 'jar-strip__spacer' : false,
    };
    await new Promise((r) => setTimeout(r, 400));
    const faded = {
      visible: node ? node.classList.contains('is-visible') : false,
  ...
  const gestureTarget = await page.evaluate(async () => {
    ...
    await new Promise((r) => setTimeout(r, 350));
    const panel = document.querySelector('.jar-settings__panel[data-category="keybindings"]');
    const row = [...document.querySelectorAll('.jar-dialog__setting')]
      .find((r) => (r.querySelector('.jar-dialog__setting-label') || {}).textContent === 'Double-tap command');
    // The panel's own group heads. The keybindings sheet reuses the same class
    // for ITS section rows, so those are excluded by their `jar-kb__section`.
    const heads = panel
      ? [...panel.querySelectorAll('.jar-settings__section-head:not(.jar-kb__section)')]
    ...
    return {
      ...
      rows: [...panel.querySelectorAll('.jar-dialog__setting')].map((r) => [
        (r.querySelector('.jar-dialog__setting-label') || {}).textContent || '',
        (r.querySelector('.jar-dialog__setting-desc') || {}).textContent || '',
      ]),
      headActions: [...panel.querySelectorAll('.jar-settings__panel-head > *')]
  ...
  const headAction = await page.evaluate(async () => {
    const btn = document.querySelector(
      '.jar-settings__panel[data-category="keybindings"] .jar-settings__head-action'
    );
    ...
    const out = {
      found: true,
      opened: true,
      settingsStillOpen: !!document.querySelector('dialog[open] .jar-settings__panel'),
  ...
  check(headAction.opened, 'the Keys head action opens Available Keys');
  ...
  // ── taken by the browser, inside Available Keys ───────────────────────────
  //
  ...
  const chords = await page.evaluate(async () => {
    ...
    const head = [...list.querySelectorAll('.jar-macros__group')]
    ...
    for (const n of list.children) {
      if (n.classList.contains('jar-macros__group')) {
        inBlock = /Taken by the browser$/.test(n.textContent);
        continue;
      }
      if (!inBlock) continue;
      if (n.classList.contains('jar-macros__aside')) { asides.push(n.textContent); continue; }
      if (n.classList.contains('jar-macros__row--meta')) { meta.push(n.textContent); continue; }
      rows.push({
        dead: (n.querySelector('.jar-macros__dead') || {}).textContent || '',
        gloss: (n.querySelector('.jar-macros__title') || {}).textContent || '',
        press: (n.querySelector('.jar-macros__keys') || {}).textContent || '',
        deadInKeys: !!n.querySelector('.jar-macros__keys .jar-macros__dead'),
      });
    }
    return {
      found: !!head,
      ...
      starred: [...win.querySelectorAll('.jar-macros__star')]
        .map((n) => (n.closest('.jar-macros__row').querySelector('.jar-macros__chord') || {}).textContent),
      rows,
      asides,
      meta,
      isMac: facts.isMac,
  ...
  check(chords.found, 'Available Keys carries the taken-by-the-browser block');
  ...
  const fullKb = await page.evaluate(async () => {
    ...
    return {
      ...
      offeredInBlock: [...win.querySelectorAll('.jar-macros__row--meta')]
