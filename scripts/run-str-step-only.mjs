...
const types = { '.js': 'text/javascript', '.mjs': 'text/javascript', '.css': 'text/css', '.html': 'text/html', '.wasm': 'application/octet-stream' };
...
await page.waitForFunction(() => !!window.BelJarEditor && !!window.BelugaClient, { timeout: 30000 });
...
const decl = strStepDecl;

const r = await page.evaluate(async (pre, decl) => {
  const code = `${pre}\n\n${decl}\n`;
  const ed = window.BelJarEditor;
  const thm = ed.theoremUnderProof(decl);
  const out = await ed.proveProgram(code, thm, (x) => window.BelugaClient.checkResult(x), { maxSteps: 200 });
  return {
...
if (r.stuck) console.log(JSON.stringify(r.stuck, null, 2));
await browser.close();
server.close();
