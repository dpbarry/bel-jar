...
const r = await page.evaluate(async (pre, decl) => {
  ...
  const out = await ed.proveProgram(code, thm, (x) => window.BelugaClient.checkResult(x), { maxSteps: 30 });
  return { complete: out.complete, steps: out.steps.map((s) => s.move), stuck: out.stuck };
}, pre, decl);

console.log(r.complete ? 'COMPLETE' : 'STUCK', r.steps.join(' -> '));
