...
import { holeHostFile, scanFileHoles } from '../js/editor-src/harpoon/scan-file-holes.mjs';
...
loadScript('js/harpoon/harpoon-ui.js');

const PG = globalThis.HarpoonGoalSections;
const PS = globalThis.ProjectSource;
expect(PG && typeof PG.buildSections === 'function', 'goal sections module loaded');
...
const model = PG.buildSections({
  ...
  computeDirLayout: (dir, filesInDir) => {
    const SL = globalThis.ExplorerSuiteLayout;
...
console.log('test-scan-file-holes: ok');
